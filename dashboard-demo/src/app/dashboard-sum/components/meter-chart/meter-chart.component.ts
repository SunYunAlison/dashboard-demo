...
export class MeterChartComponent implements OnInit {
  ...
  toggleFlip() {
    console.log("before click",this.flip);
    this.flip = (this.flip == 'inactive') ? 'active' : 'inactive';
    console.log("after click",this.flip);
    setTimeout(() => {
      this.display = (this.flip == 'inactive') ? 'front' : 'back';
  ...
  ngOnInit(): void {
    ...
    if (this.title === 'Current') {
      ...
      this.sensor = this.sensorInfo[0];
    }
    else if (this.title === 'Temperature') {
      this.draw1StageMeterChart(this.data);
      this.sensor = this.sensorInfo[1];
    }
    else {
      this.draw1StageMeterChart(this.data);
      this.sensor = this.sensorInfo[2];
    }
  }
